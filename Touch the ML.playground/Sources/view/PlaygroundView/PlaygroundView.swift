...
public struct PlaygroundView: View {
    ...
    var canAdjustModel: Bool
    var canAdjustLayer: Bool
    var canUseSetting: Bool
    var canChangeDataset: Bool
    var scale: CGFloat
    var modelDesc: [Int]
    
    public init(
        adjModel: Bool = true, adjLayer: Bool = true, useSetting: Bool = true, chgData: Bool = true, scale: CGFloat = 1.0,
        modelDesc: [Int] = [2], type: DataGenerator.T = .linecut
    ) {
        canAdjustModel = adjModel
        canAdjustLayer = adjLayer
        canUseSetting = useSetting
        canChangeDataset = chgData
        self.scale = scale
        self.modelDesc = modelDesc
        vm.model.desc = modelDesc
        DataGenerator.dataType = type
        vm.model.data = DataGenerator.getTrainingData()
        vm.newModelGenerated()
    }
